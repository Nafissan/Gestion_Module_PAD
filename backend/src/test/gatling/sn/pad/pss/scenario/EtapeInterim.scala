...
class EtapeInterim {
    ...
    object EtapeInterim {
        val enregistrer = exec(http("Enregitrer les informations d'un etapeInterims")
            .post("/etapeInterims")
            .body(
                StringBody(
                    """{
                        ...
                        "date": null,
                        "action": "Supprimer",
                        "titre": "Responsable securitÃ©",
                        "fonction": "SÃ©curitÃ©",
                        "structure": "DD",
                        "matricule": "607117",
                        "prenom": "Mamadou Mendy",
                        "nom": "BaldÃ©",
                        "etat": "SAISI",
                        "createdAt": null,
                        "updatedAt": null,
            ...
            .check(status.is(201))
            .check(bodyString.saveAs("etapeInterim_enregistre"))
            .check(jsonPath("$.id").saveAs("id"))
            .transformResponse(log)) 

        val liste = exec(http("Liste des etapeInterims")
            .get("/etapeInterims")
            .check(status.is(200))
        )

        val modifier = exec(http("Modifier les informations d'un etapeInterims")
            .put("/etapeInterims")
            .body(
                StringBody(
                   "${etapeInterim_enregistre}"
                )
            ).asJson
            .check(status.is(200))
        )

        val supprimer = exec(http("Supprimer un etapeInterims")
            .delete("/etapeInterims")
            .body(
                StringBody(
                    "${etapeInterim_enregistre}"
                )
        ...
        val rechercherParIdentifiant = exec(http("Recherche un etapeInterims par son identifiant")
            .get("/etapeInterims/${id}")
            .check(status.is(200))
        )
        
        val liste100 = exec(http("Liste des etapeInterims - 100 répétition avec un intervalle de 2 milliseconds -")
            .get("/etapeInterims")
            .check(status.is(200))
    ...
    val crud = scenario("CRUD ETAPEINTERIM").exec(EtapeInterim.enregistrer, EtapeInterim.liste, EtapeInterim.rechercherParIdentifiant, EtapeInterim.modifier, EtapeInterim.supprimer)
    val charges = scenario("Tests de charges pour la ressource EtapeInterim").exec(EtapeInterim.liste100)

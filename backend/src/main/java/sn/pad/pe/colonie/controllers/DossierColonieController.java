...
public class DossierColonieController {
    ...
    @ApiOperation(value = "Récupération d'un dossier colonie par ID", response = DossierColonieDTO.class)
    ...
    @GetMapping("/dossiersColonies/{id}")
    public ResponseEntity<DossierColonieDTO> getDossierColonieById(@PathVariable Long id) {
        return ResponseEntity.ok().body(dossierColonieService.getDossierColonieById(id));
    }
